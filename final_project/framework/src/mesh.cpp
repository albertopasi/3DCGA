    ...
    for (const auto& shape : inShapes) {
        ...
        for (size_t endTriangle = 0; endTriangle < shape.mesh.indices.size() / 3; ++endTriangle) {
            ...
            for (size_t i = startTriangle * 3; i != endTriangle * 3; i += 3) {
                ...
                // **** Calculate the tangent and bitangent for the normal mapping *****

                //Calculate position delta and texture coord delta (UVs delta)
                glm::vec3 deltaPos1 = mesh.vertices[triangle[1]].position - mesh.vertices[triangle[0]].position;
                glm::vec3 deltaPos2 = mesh.vertices[triangle[2]].position - mesh.vertices[triangle[0]].position;
                ...
                glm::vec3 tangent = f * (deltaUV2.y * deltaPos1 - deltaUV1.y * deltaPos2);
                glm::vec3 bitangent = f * (-deltaUV2.x * deltaPos1 + deltaUV1.x * deltaPos2);

                //Save tangent and bitangent in the vertexes
                for(unsigned j = 0; j < 3; j++){
